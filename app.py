...
if not st.session_state.user:
    ...
    # 6. Growth Chart
        ...
        if records:
            ...
            fig.update_layout(
                xaxis=dict(tickangle=-45), margin=dict(l=20, r=20, t=40, b=20)
            )
    ...
    # 8. Smart Analysis
        ...
        if records:
            ...
            st.subheader("💡 تحلیل و پیشنهاد هوشمند:")
            if avg_all >= 85:
                st.success(
                    "🎉 **عملکرد شما فوق‌العاده است!** تداوم خود را حفظ کنید و"
                    " روی اهداف بزرگ‌تر تمرکز نمائید."
                )
            elif avg_all >= 70:
                st.success(
                    "👍 **وضعیت خوبی دارید.** با کمی دقت روی برنامه‌های روزانه"
                    " می‌توانید به سطح عالی برسید."
                )
            elif avg_all >= 50:
                st.warning(
                    "⚠️ **عملکرد شما متوسط است.** پیشنهاد می‌شود اهداف روزانه"
                    " را کوچکتر و قابل دستیابی‌تر کنید."
                )
            else:
                st.error(
                    "🔴 **عملکرد نیاز به بهبود جدی دارد.** انجام دادن حتی یک"
                    " کار کوچک در روز می‌تواند روند شما را مثبت کند. قدم‌های"
                    " کوچک بردارید!"
                )
        else:
    ...
    # 9. Journal
        ...
        mood = st.selectbox(
            "حالت روحی / Mood", mood_options, index=default_mood_idx
        )
        default_note = (
            curr_j[1] if curr_j and len(curr_j) > 1 and curr_j[1] else ""
        )
